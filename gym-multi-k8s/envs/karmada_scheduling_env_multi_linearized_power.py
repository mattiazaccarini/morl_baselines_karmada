...
from envs.utils import DeploymentRequest, get_5gcore_deployment_list, save_to_csv_power, sort_dict_by_value, \
...
COST = 'cost'
MAX_COST = 72  # Defined based on the max cost in DEFAULT_CLUSTER_TYPES
MIN_COST = 2  # Defined based on the min cost in DEFAULT_CLUSTER_TYPES
...
class KarmadaSchedulingEnvMultiLinearizedPower(gym.Env):
    ...
    def __init__(self, num_clusters=DEFAULT_NUM_CLUSTERS,
        ...
        self.gini_weight = 0.25
        self.power_weight = 0.25
    ...
    def step(self, action):
        ...
        if self.current_step == self.episode_length:
            ...
            logging.info("[Step] Total Power Consumption: {}".format(self.total_power_consumption))
            save_to_csv_power(self.file_results, self.episode_count,
    ...
    def get_reward(self):
        ...
        # Multi-Objective Reward Function
            ...
            if self.penalty:
                ...
                power_reward = normalize(self.total_power_consumption, 
                               self.min_power_consumption, self.max_power_consumption)

                reward = self.latency_weight * (1 - lat) + self.cost_weight * (1 - cost) + self.gini_weight * (1 - gini) + self.power_weight * (1 - power_reward)
