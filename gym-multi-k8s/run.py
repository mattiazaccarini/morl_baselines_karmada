...
from wrappers.karmada_env_wrapper import KarmadaMOWrapper
...
def get_env(env_name, num_clusters, reward_function, min_replicas, max_replicas):
    ...
    if env_name == "karmada":
        ...
        wrapped_env = KarmadaMOWrapper(env)

        env.reset()
        _, _, _, info = env.step(0)
        info_keywords = tuple(info.keys())
        env = SubprocVecEnv([lambda: KarmadaMOWrapper(num_clusters=num_clusters, arrival_rate_r=100,
